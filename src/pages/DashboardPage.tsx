  ...
  return (
    <div className="rounded-xl border border-border bg-surface p-6 flex flex-col">
      ...
      {isLoading ? (
        ...
        <div className="flex-1 flex items-center justify-center py-8">
          <p className="text-sm text-muted">Nenhum evento registrado até o momento.</p>
        </div>
      ) : (
      ...
      <div className="mt-4 pt-3 border-t border-border/50">
        <p className="text-xs text-muted text-center">
          ...
          <br />
          Em evolução: filtros, busca e relatórios completos de auditoria.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <PageContainer title="Painel">
      <p className="text-muted -mt-2">
        Boas-vindas, <span className="text-text font-medium">{user?.name || user?.email}</span>. Confira o panorama da plataforma.
      </p>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div className="rounded-xl border border-border bg-surface p-6">
              ...
              <div className="space-y-3">
                ...
                <BarRow label="Diretores escolares" value={stats?.users.directors ?? 0} total={stats?.users.total ?? 1} color="bg-warning" />
                <BarRow label="Administradores" value={stats?.users.admins ?? 0} total={stats?.users.total ?? 1} color="bg-accent" />
              </div>
            </div>
